            ...
            <template v-for="task in carousel">
              ...
              >
                <template slot="content">
                  ...
                  <p class="title" :title="task.taskName" :style="{transform: 'scale('+ cardScale +')'}">
                    {{ task.taskName }}</p>
                  <p style="height: 15px">
            ...
            <template v-if="carousel.length<4">
              <module-card v-for="placeholder in 4-carousel.length" shadow="always" :key="placeholder">
                <template slot="content">
                  ...
                  <span style="color: #7390CB;text-align: center;padding-right: 8px;"
                        :style="{transform: 'scale('+ cardScale +')'}">暂无数据</span>
                </template>
              </module-card>
            </template>
          </div>
...
export default {
  ...
  methods: {
    ...
    stageDetailView(stage, error) {
      ...
      if (error) {
        this.$refs.monitorLeader.gridController.gridApi.setFilterModel({
          stepStatus: {
            type: 'set',
            values: ['03', '04']
          }
