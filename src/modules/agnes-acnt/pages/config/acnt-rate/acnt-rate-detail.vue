<template>
    <div>
        ...
        <el-form-item label="网点代码" prop="branchCode">
            <gf-input v-model="form.branchCode" placeholder="网点代码" :max-byte-len="120"/>
        </el-form-item>
        <el-form-item label="利率" prop="rate">
            <gf-input v-model="form.rate" placeholder="利率" :max-byte-len="120"/>
        </el-form-item>
        <el-form-item label="状态" prop="status">
            <gf-dict-select v-model.trim="form.status" dict-type="AGNES_RELEASE_STATUS" placeholder="状态" :max-byte-len="120"/>
        ...
        data() {
            return {
                ...
                rules: {
                    ...
                    // 'branchName': [{required: true, message: "网点名称不能为空"}],
        ...
        methods: {
            async getOptionData(){
                try {
                    ...
                    this.branchList = groupOption.data
                } catch (reason) {
            ...
            async onSave() {
                ...
                try {
                    const p = this.$api.rateDefApi.saveRate(this.form);
