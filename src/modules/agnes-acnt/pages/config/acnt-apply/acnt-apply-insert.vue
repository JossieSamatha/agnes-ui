...
<!--        <el-divider v-if="showChange"></el-divider>-->
        ...
        <el-form ref="taskDefForm" class="acnt-apply-insert-form" :model="detailForm"
                ...
                <template slot="content">
                    <div class="line">
                        <el-form-item label="账户类型" prop="typeCode">
                            <el-select class="multiple-select" v-model="detailForm.typeCode"
                                       :disabled="mode!=='registration' && mode!=='editData'"
                      ...
                      <el-form-item label="业务类型" prop="bizType">
                        <gf-dict :disabled="true" filterable clearable v-model="detailForm.bizType"
                                 dict-type="AGNES_ACNT_BIZ_TYPE"
                    ...
                    <div class="line">
                        <el-form-item label="业务描述" prop="baseDesc">
                          <gf-input v-model="detailForm.baseDesc" type='textarea' placeholder="业务描述" :max-len="200"
                                    :disabled="mode!=='registration' && mode!=='editData'"
                        ...
                        <el-form-item label="业务受理部门" prop="baseAcceptDept">
                          <gf-dict filterable clearable v-model="detailForm.baseAcceptDept"
                    ...
                    <div class="line">
                        <el-form-item label="业务受理组" prop="baseAcceptGroup">
                          <el-select class="multiple-select" v-model="detailForm.baseAcceptGroup"
                                     filterable clearable :disabled="mode!=='registration' && mode!=='editData'"
                                     placeholder="请选择"
                    ...
                    <div class="line">
                      <el-form-item label="基金名称" prop="productName">
                        <gf-input type='textarea' :disabled="true" v-model.trim="detailForm.productName"
                                  placeholder="基金名称"
                    ...
                    <div class="line">
                      <el-form-item label="账户状态" prop="acntStatus"
                                    v-if="this.detailForm.bizType === '02' || this.detailForm.bizType === '05'"
                                    width="100%">
                ...
                <template slot="content">
                    <el-form-item v-if="showRules.accNo&&showRules.accNo.isShow" label="证券账号" prop="accNo">
                        <div class="rule-table">
                          <el-table header-row-class-name="rule-header-row"
                                    ...
                                    border stripe
                                    style="width: 100%"
                                    :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.accNos ? '1px solid red':''}">
                    ...
                    <el-form-item v-if="showRules.fundAccNo&&showRules.fundAccNo.isShow" label="资金帐号" prop="fundAccNo">
                        <div class="rule-table">
                          <el-table header-row-class-name="rule-header-row"
                                    ...
                                    :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.fundAccNos ? '1px solid red':''}">
                    ...
                    <div class="line wrap">
                      <el-form-item v-if="showRules.acntName&&showRules.acntName.isShow" label="账户名称" prop="acntName">
                        <gf-input type='textarea' v-model.trim="detailForm.acntName" placeholder="账户名称"
                                  :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.acntName ? '1px solid red':''}"/>
                      </el-form-item>
                      <el-form-item v-if="showRules.acntShortName&&showRules.acntShortName.isShow" label="账户简称"
                                    prop="acntShortName">
                        <gf-input type='textarea' v-model.trim="detailForm.acntShortName" placeholder="账户简称"
                                  :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.acntShortName ? '1px solid red':''}"/>
                      </el-form-item>
                      <el-form-item v-if="showRules.fundAccName&&showRules.fundAccName.isShow" label="资金账户名称"
                                    prop="fundAccName">
                        <gf-input type='textarea' v-model.trim="detailForm.fundAccName" placeholder="资金账户名称"
                                  :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.fundAccName ? '1px solid red':''}"/>
                      </el-form-item>
                      ...
                      <el-form-item v-if="showRules.market&&showRules.market.isShow" label="市场" prop="market">
                        <gf-dict filterable clearable v-model="detailForm.market" dict-type="AGNES_ACNT_MARKET"
                                 :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.market ? '1px solid red':''}"/>
                      </el-form-item>

                      <el-form-item v-if="showRules.acntStatus&&showRules.acntStatus.isShow" label="账户状态"
                                    prop="acntStatus">
                        <gf-dict filterable clearable v-model="detailForm.acntStatus"
                                 dict-type="AGNES_ACNT_CURRENCY_TYPE"
                                 :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.acntStatus ? '1px solid red':''}"/>
                      </el-form-item>

                      <el-form-item v-if="showRules.settlementNo&&showRules.settlementNo.isShow" label="清算编号"
                                    prop="settlementNo">
                        <gf-input v-model.trim="detailForm.settlementNo" placeholder="清算编号"
                                  :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.settlementNo ? '1px solid red':''}"/>
                      </el-form-item>
                        <el-form-item v-if="showRules.bigPayNo&&showRules.bigPayNo.isShow" label="大额支付号" prop="bigPayNo">
                            ...
                            <div class="line">
                              <el-select class="multiple-select" v-model="detailForm.bigPayNo" style="width: 90%"
                                         ...
                                         :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.bigPayNo ? '1px solid red':''}">
                                <gf-filter-option
                      ...
                      <el-form-item v-if="showRules.region&&showRules.region.isShow" label="账户所属地区" prop="region">
                        <gf-input v-model.trim="detailForm.region" placeholder="账户所属地区"
                                  :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.region ? '1px solid red':''}"/>
                      </el-form-item>
                      <el-form-item v-if="showRules.currency&&showRules.currency.isShow" label="币种" prop="currency">
                        <gf-dict filterable clearable v-model="detailForm.currency" dict-type="AGNES_ACNT_CURRENCY_TYPE"
                                 :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.currency ? '1px solid red':''}"/>
                      </el-form-item>
                        ...
                        <el-form-item v-if="showRules.rateCode&&showRules.rateCode.isShow" label="利率" prop="rateCode">
                          <el-select v-model="detailForm.rateCode"
                                     ...
                                     filterable
                                     placeholder="请选择"
                                     :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.rateCode ? '1px solid red':''}">
                            <gf-filter-option
                      ...
                      <el-form-item v-if="showRules.stampInfo&&showRules.stampInfo.isShow" label="预留印鉴信息"
                                    prop="stampInfo">
                        <gf-input type='textarea' v-model="detailForm.stampInfo" placeholder="预留印鉴信息"
                                  :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.stampInfo ? '1px solid red':''}"/>
                      </el-form-item>


                      <el-form-item v-if="showRules.stampLegalPersonInfo&&showRules.stampLegalPersonInfo.isShow"
                                    label="印鉴法人变更情况" prop="stampLegalPersonInfo">
                        <gf-input type='textarea' v-model="detailForm.stampLegalPersonInfo" placeholder="印鉴法人变更情况"
                                  :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.stampLegalPersonInfo ? '1px solid red':''}"/>
                      </el-form-item>

                      <el-form-item v-if="showRules.threeLicenseInfo&&showRules.threeLicenseInfo.isShow"
                                    label="三证合一变更情况" prop="threeLicenseInfo">
                        <gf-input type='textarea' v-model="detailForm.threeLicenseInfo" placeholder="三证合一变更情况"
                                  :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.threeLicenseInfo ? '1px solid red':''}"/>
                      </el-form-item>
                      ...
                      <el-form-item v-if="showRules.acntPurpose&&showRules.acntPurpose.isShow" label="账户用途描述"
                                    prop="acntPurpose">
                        <gf-input type='textarea' v-model="detailForm.acntPurpose" placeholder="账户用途描述"
                                  :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.acntPurpose ? '1px solid red':''}"/>
                      </el-form-item>
                        ...
                        <el-form-item  v-if="showRules.provisionBankAcntIds&&showRules.provisionBankAcntIds.isShow" label="备付金账户对应的银行账户" prop="baseOperator">
                          <el-select style="width: 100%" class="multiple-select"
                                     v-model="detailForm.provisionBankAcntIds"
                                     filterable clearable multiple
                                     placeholder="请选择"
                                     :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.provisionBankAcntIds ? '1px solid red':''}">
                            <gf-filter-option
                        ...
                        <el-form-item v-if="showRules.cardCorporateAcntId&&showRules.cardCorporateAcntId.isShow" label="卡账户对应的对公户" prop="productCode">
                          <el-select style="width: 100%" class="multiple-select"
                                     v-model="detailForm.cardCorporateAcntId"
                                     filterable clearable
                                     placeholder="请选择"
                                     :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.cardCorporateAcntId ? '1px solid red':''}">
                            <gf-filter-option
                                v-for="item in acntList"
                                :key="item.acntId"
                                :label="item.acntLabel"
                        ...
                        <el-form-item v-if="showRules.maturityDt&&showRules.maturityDt.isShow" label="到期提醒" prop="maturityDt">
                          <el-date-picker
                              ...
                              value-format="yyyy-MM-dd"
                              placeholder="到期提醒"
                              :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.maturityDt ? '1px solid red':''}">
                          </el-date-picker>
                        </el-form-item>

                      <el-form-item v-if="showRules.futuresCompany&&showRules.futuresCompany.isShow" label="期货公司"
                                    prop="futuresCompany">
                        <gf-input type='textarea' v-model="detailForm.futuresCompany" placeholder="期货公司"
                                  :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.futuresCompany ? '1px solid red':''}"/>
                      </el-form-item>

                      <el-form-item v-if="showRules.limits&&showRules.limits.isShow" label="额度" prop="limits">
                        <gf-input type='textarea' v-model="detailForm.limits" placeholder="额度"
                                  :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.limits ? '1px solid red':''}"/>
                      </el-form-item>
                        ...
                        <el-form-item v-if="showRules.bankLinkMan&&showRules.bankLinkMan.isShow" label="银行/开户时对方联系人"
                            ...
                            <div class="line">
                              <el-select class="multiple-select" v-model="detailForm.bankLinkMan"
                                         ...
                                         placeholder="请选择"
                                         style="width: 90%"
                                         :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.bankLinkMan ? '1px solid red':''}">
                                <gf-filter-option
                      ...
                      <el-form-item v-if="showRules.other&&showRules.other.isShow" label="其他" prop="other">
                        <gf-input type='textarea' v-model="detailForm.other" placeholder="其他"
                                  :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.other ? '1px solid red':''}"/>
                      </el-form-item>

                      <el-form-item v-if="showRules.remark&&showRules.remark.isShow" label="备注" prop="remark">
                        <gf-input type='textarea' v-model="detailForm.remark" placeholder="备注"
                                  :style="{border: detailFormBefore.modifyFileds && detailFormBefore.modifyFileds.remark ? '1px solid red':''}"/>
                      </el-form-item>
            ...
            <div v-if="detailForm.isSendOa==='1'" class="line">
                <el-form-item v-if="detailForm.isSendOa==='1'" label="OA用印文件" prop="fileTable">
                    <div class="rule-table">
                        <acc-ecm-upload style="width: 100%;"
                                        :disabled="true"
                                        :showRemove="false"
  ...
  props: {
    ...
    row: Object,
        ...
        beforeMount() {
            ...
            }else if(this.mode=='editData'){
                this.detailForm.bizType='05'
            }else{
        ...
        methods: {
          ...
          deleteAccRuleRow(rowIndex) {
            ...
            async getOptionData(){
                try {
                    ...
                    //20210105 发起编辑，补充关联信息
                    if(this.mode === 'editData'){
                        this.detailForm.bankLinkMan = this.$lodash.cloneDeep(this.detailFormBefore.bankLinkMan);
                        this.detailForm.provisionBankAcntIds = this.$lodash.cloneDeep(this.detailFormBefore.provisionBankAcntIds);
                        this.accNoList = this.$lodash.cloneDeep(this.detailFormBefore.accNoList);
                        this.moneyAccNoList = this.$lodash.cloneDeep(this.detailFormBefore.moneyAccNoList);
                    }
          ...
          async temporarySave() {
            ...
            try {
              ...
              if (form.processStatus === '06' && !this.$lodash.isEmpty(form.acntName)) {
                const check = await this.$api.acntApplyApi.checkAcntName(form);
                if (form.bizType.match(/01|04/) && check.code === 'exit') {
                  const ok = await this.$msg.ask(`该账户名称存在重复, 是否继续?`);
                  if (!ok) {
