export function toColumn(menus) {
    ...
    for(let i=0;i<menus.length;i++) {
        let first = menus[i];
        let row = {code:first.resCode, title:first.resName,actionUrl:first.actionUrl};
        if(!first.children || first.children.length==0){
            ...
            for(let j=0;j<first.children.length;j++){
                ...
                let column = {code:second.resCode, title:second.resName, menus:new Array(),actionUrl:second.actionUrl}
                //三级菜单 menus
                for(let k=0;k<second.children.length;k++) {
                    let third = second.children[k];
                    column.menus.push({code:third.resCode, title:third.resName, view:third.resCode,actionUrl:third.actionUrl});
                }
                columns.push(column);
            }
