...
export default {
    ...
    toggleDraggable({commit, state}, {id}) {
        if (!state.datavComps[id].draggable) {
    ...
    toggleResizable({commit, state}, {id}) {
        if (!state.datavComps[id].resizable) {
    ...
    toggleParentLimitation({commit, state}, {id}) {
        if (!state.datavComps[id].parentLim) {
    ...
    toggleSnapToGrid({commit, state}, {id}) {
        if (!state.datavComps[id].snapToGrid) {
    ...
    changeXLock({commit, state}, {id}) {
        switch (state.datavComps[id].axis) {
            case 'both':
    ...
    changeZToBottom({commit, state}, {id}) {
        if (state.datavComps[id].zIndex === 1) {
            return
        }
        ...
        for (let i = 0, l = state.datavComps.length; i < l; i++) {
            if (i !== id) {
                if(state.datavComps[i].zIndex === state.datavComps.length){
                    continue
                }
                commit(types.CHANGE_ZINDEX, {id: i, zIndex: state.datavComps[i].zIndex + 1});
            }
        }
    },

    changeZToTop({commit, state}, {id}) {
        if (state.datavComps[id].zIndex === state.datavComps.length) {
            return
        }

        commit(types.CHANGE_ZINDEX, {id, zIndex: state.datavComps.length});

        for (let i = 0, l = state.datavComps.length; i < l; i++) {
            if (i !== id) {
                if(state.datavComps[i].zIndex === 1){
                    continue
                }
                commit(types.CHANGE_ZINDEX, {id: i, zIndex: state.datavComps[i].zIndex - 1});
            }
